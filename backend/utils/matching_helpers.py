import re
from backend.utils.normalizers import extract_pipe_size, extract_fitting_size, extract_thread_size
from backend.constants import PRODUCT_TYPE_MARKERS, CLIENT_HINTS, CRITICAL_TYPES
...
def filter_by_category(matches: list, client_cat: str | None) -> list:
    ...
    if client_cat == 'pert':
        ...
        # Default sewer logic
        sku_202 = [m for m in matches if get_product(m).get('sku', '').startswith('202')]
        if sku_202:
            return sku_202
        filtered = [m for m in matches
