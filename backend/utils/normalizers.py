...
def _load_json_config(filename: str) -> dict:
    try:
        ...
        print(f"⚠️ Error loading config {filename}: {e}")
        return {}


_synonyms = _load_json_config("synonyms.json")
MATERIAL_SYNONYMS = _synonyms.get("material", {})
PRODUCT_SYNONYMS = _synonyms.get("product", {})

_pipe_sizes = _load_json_config("pipe_sizes.json")
# Convert keys to int (JSON keys are always strings)
PIPE_MM_TO_INCH = {int(k): v for k, v in _pipe_sizes.items() if k.isdigit()}

if not MATERIAL_SYNONYMS:
    # Fallback to defaults if config missing (essential for tests/runtime safety)
    MATERIAL_SYNONYMS = {
        "пп": "полипропилен",
        "pp": "полипропилен",
        "пэ": "полиэтилен",
...
def expand_synonyms(text: str) -> str:
    ...
    sorted_synonyms = sorted(
        PRODUCT_SYNONYMS.items(), key=lambda x: len(x[0]), reverse=True
    )
...
def normalize_name(name: str) -> str:
    ...
    # СНАЧАЛА нормализуем каталожный формат резьбы (Jakko: "с вн. рез.", "с нар. рез.")
    # Это нужно сделать ДО expand_synonyms, иначе будет двойное "рез рез"
    result = re.sub(
        r"\bс\s+вн\.?\s*рез\.?\b", "вн рез", result
    )  # "с вн. рез." → "вн рез"
    result = re.sub(
        r"\bс\s+нар\.?\s*рез\.?\b", "нар рез", result
    )  # "с нар. рез." → "нар рез"

    # Расширяем синонимы материалов и типов (ВР→вн рез, НР→нар рез и др.)
    result = expand_synonyms(result)

    # Нормализуем полные формы резьбы (если остались после synonyms)
    # "с внутренней резьбой" → "вн рез", "с наружной резьбой" → "нар рез"
    result = re.sub(r"\bс?\s*внутр\w*\s*резьб\w*", "вн рез", result)
    result = re.sub(r"\bс?\s*наруж\w*\s*резьб\w*", "нар рез", result)
    # Дедупликация: "вн рез рез" → "вн рез" (если образовалось от ВР + regex)
    result = re.sub(r"\b(вн|нар)\s+рез\s+рез\b", r"\1 рез", result)
    ...
    result = re.sub(
        r"\bхомут\s+([d\d]+\s*мм?|\d+)?\s*с\s+защелк\w*", convert_clamp_to_clip, result
    )
    ...
    result = re.sub(
        r"\(ремонтная\)", "ремонтная", result
    )  # убираем скобки, слово оставляем
    ...
    result = re.sub(
        r"\b(45|67|87|90)\s*(?:град(?:ус)?\.?|гр\.?)\b",
        r"\1",
        result,
        flags=re.IGNORECASE,
    )
    ...
    if "компрессионн" in result:
        ...
        result = re.sub(
            r"\bкомпрессионн\w*\s+компрессионн\w*\b", "компрессионный", result
        )
        result = " ".join(result.split())
    return result
...
def extract_fitting_size(name: str) -> tuple[int, ...] | None:
    ...
    clean_name = re.sub(
        r"\b(45|67|87|90)\s*(?:°|град(?:ус)?\.?|гр\.?)(?:\s|$|(?=[^\d]))",
        " ",
        clean_name,
        flags=re.IGNORECASE,
    )
    ...
    m = re.search(
        r"\b(\d{2,3})\s*[-/×xхXХ*]\s*(\d{2,3})(?:\s*[-/×xхXХ*]\s*(\d{2,3}))?\b",
        clean_name,
    )
    if m:
    ...
    types_pattern = (
        r"(муфт|заглуш|ревизи|крестовин|тройник|переход|отвод|угол|сифон|клипс)"
    )
    if re.search(types_pattern, name_lower):
        ...
        numbers = [
            int(n)
            for n in re.findall(r"(?:\b|d)(\d{2,3})(?:мм)?\b", name_lower)
            if 20 <= int(n) <= 200
        ]
        if numbers:
