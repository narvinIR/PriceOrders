...
MATERIAL_SYNONYMS = {
    ...
    'pert': 'полиэтилен',
    'pe-rt': 'полиэтилен',
    # ПНД = полиэтилен низкого давления (компрессионные фитинги)
    'пнд': 'полиэтилен',
    'hdpe': 'полиэтилен',
}
...
PRODUCT_SYNONYMS = {
    ...
    'н р': 'наружная резьба',
    'н/р': 'наружная резьба',
    'нр': 'наружная резьба',  # НР без разделителя
    'вр': 'внутренняя резьба',  # ВР без разделителя
    # Сокращения клиентов
...
def extract_thread_size(name: str) -> tuple[int, str] | None:
    """
    Извлечь размер с резьбой: "32*1" → (32, "1")
    ...
    # Проверяем контекст - должна быть муфта с резьбой (НР/ВР/разъемная)
    has_thread_context = any(x in name_lower for x in [
        'нр', 'вр', 'н/р', 'в/р', 'нар', 'внутр', 'разъем', 'американ',
        'нар.рез', 'вн.рез', 'нар рез', 'вн рез'
    ])
    ...
    if m:
        ...
        # Валидация размера мм (20-63) и дюймов резьбы (не > 2)
        if 20 <= mm <= 63:
            # Проверяем что это действительно дюймы резьбы
            # 1, 1/2, 3/4, 1/4, 11/4, 11/2, 2 - корректные
            # 20, 25, 32, 40 - это размеры фитингов, не резьба!
            if inches in ['1', '2', '1/2', '3/4', '1/4', '11/4', '11/2', '21/2']:
                return (mm, inches)
    return None
...
def extract_fitting_size(name: str) -> tuple[int, ...] | None:
    ...
    # Примечание: \b в конце не работает после °, поэтому используем (?:\s|$|(?=[^\d]))
    clean_name = re.sub(r'\b(45|67|87|90)\s*(?:°|град(?:ус)?\.?|гр\.?)(?:\s|$|(?=[^\d]))', ' ', name, flags=re.IGNORECASE)
    ...
    types_pattern = r'(муфт|заглуш|ревизи|крестовин|тройник|переход|отвод|угол|сифон|клипс|хомут)'
