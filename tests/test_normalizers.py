...
class TestNormalizeName:
    ...
    def test_normalize_separators(self):
        """Все разделители → ×"""
        assert '×' in normalize_name('Труба 110×2000') or '110 2000' in normalize_name('Труба 110×2000')
...
class TestExpandSynonyms:
    ...
    def test_thread_vn_rez(self):
        ...
        assert 'внутренняя резьба' in result

    def test_thread_nar_rez(self):
        result = expand_synonyms('муфта с нар.рез.')
        assert 'наружная резьба' in result
